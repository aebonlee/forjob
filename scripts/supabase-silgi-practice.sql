...
-- ============================================
--
-- 기존 forjob_exam_sessions 테이블을 재활용하여
...
-- forjob_exam_sessions 테이블이 없는 경우에만 생성
...
CREATE INDEX IF NOT EXISTS idx_forjob_exam_sessions_user_type
    ON forjob_exam_sessions (user_id, exam_type);

-- completed_at 내림차순 인덱스 (최근 기록 조회)
CREATE INDEX IF NOT EXISTS idx_forjob_exam_sessions_completed
...
-- WHERE table_name = 'forjob_exam_sessions'
...
-- 전체 실기 연습 기록
-- SELECT * FROM forjob_exam_sessions
