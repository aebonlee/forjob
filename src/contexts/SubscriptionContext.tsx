...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (cached) {
      ...
      // 캐시 사용 시 loading=false 즉시 → 백그라운드에서 DB 갱신
      setLoading(false);
    } else {
      setLoading(true);
    }
    ...
    // 실패한 주문 자동 복구
    try {
      const raw = localStorage.getItem('jp_failed_order');
      if (raw && supabase) {
        ...
        if (stillFailed.length > 0) {
          localStorage.setItem('jp_failed_order', JSON.stringify(stillFailed));
        } else {
          localStorage.removeItem('jp_failed_order');
    ...
    try {
      ...
      if (seq !== refreshSeq.current) return;
      // 캐시가 없고 에러 발생 시 localStorage 재확인
      if (!cached) {
